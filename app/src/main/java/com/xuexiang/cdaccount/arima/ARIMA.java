 ...
 * @return ģ�͵�AICֵ
 */
	public double getmodelAIC(Vector<double[]> para, @NotNull double[] stdoriginalData, int type)
	{
		double temp;
		double temp2;
		double sumerr=0;
		...
		if(type==1)
		{
			...
			for(int k=q-1;k<n;k++)
			{
				...
				//��������ʱ�̵�����
				if (q - 1 >= 0) System.arraycopy(err, 0, err, 1, q - 1);
	...
	public int predictValue(int p,int q)
	{
		...
		if(p==0)
		{
			...
			for(int k=q;k<n;k++)
			{
				...
				//��������ʱ�̵�����
				if (q >= 0) System.arraycopy(err, 0, err, 1, q);
				err[0]=random.nextGaussian()*Math.sqrt(maPara[0]);
			}
			predict=(int)(temp); //����Ԥ��
		}
